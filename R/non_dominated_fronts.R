...
# nondominatedfronts <- function(population, popSize, fitness) { pop_count <- 0 DominatedCount =
# vector('list', popSize) DominationSet = vector('list', popSize) front <- vector('list', popSize) q <- c()
# front_index = 1 for (i in seq_len(popSize)) { DominatedCount[[i]] <- 0 } for (i in seq_len(popSize)) {
# for (j in seq_len(popSize)) { if(i != j & i < j) { if (all(fitness[i, ] <= fitness[j, ]) &&
# any(fitness[i, ] < fitness[j, ])) { DominationSet[[i]] <- c(DominationSet[[i]], j) DominatedCount[[j]] <-
# DominatedCount[[j]] + 1 } if (all(fitness[j, ] <= fitness[i, ]) && any(fitness[j, ] < fitness[i, ])) {
# DominationSet[[j]] <- c(DominationSet[[j]], i) DominatedCount[[i]] <- DominatedCount[[i]] + 1 } } } if
# (DominatedCount[[i]] == 0){ q <- c(q,i) front[[i]] <- front_index pop_count <- (pop_count + 1) } } f <-
# list() f[front_index] <- list(sort(q)) while (TRUE) { q <- c() for (i in f[[front_index]]) { for(j in
# DominationSet[[i]]) { DominatedCount[[j]] <- DominatedCount[[j]] - 1 if (DominatedCount[j] == 0) { q <-
# c(q, j) front[[j]] <- front_index + 1 pop_count <- (pop_count + 1) } } } if (is.null(q)) break
# front_index <- front_index + 1 f[front_index] <- list(sort(q)) } out <- list(F = f, Front = front)
# return(out) }
